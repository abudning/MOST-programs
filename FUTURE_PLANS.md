...
- V16.42/`1.7.643` passed office testing except Reconcile. V16.43/`1.7.644` repaired Reconcile. V16.44/`1.7.645` used an incorrect repeated share name and is superseded. V16.45/`1.7.646` uses and locally records the correct mapped path `K:\Program Files\MOSt\databases\` after verifying `oms.dbc` exists there. V16.46/`1.7.647` additionally routes all Patients shared cursors through the configured database containers instead of legacy local relative paths.

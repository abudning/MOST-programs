...
lcBackup=lcRoot+"backup_v16_44_xp_k_database_path_20260913\"
lcLog=lcOutput+"build_v16_44_xp_k_database_path.log"
...
COPY FILE "C:\Users\abudn\Documents\Codex\2026-09-12\wh\work\openletterwriter_v16_44.prg" TO (lcPrograms+"openletterwriter.prg")
COPY FILE "C:\Users\abudn\Documents\Codex\2026-09-12\wh\work\letterpatientreloadtimer_v16_44.prg" TO (lcPrograms+"letterpatientreloadtimer.prg")
COPY FILE "C:\Users\abudn\Documents\Codex\2026-09-12\wh\work\rxcenter_v16_44.prg" TO (lcPrograms+"rxcenter.prg")
...
* V16.44: prefer the new mapped shared database location and persist it locally.
lcText=STRTRAN(lcText, ;
'path_to_data = ADDBS(ALLTRIM(parameter2.data_files))', ;
'path_to_data = ADDBS(ALLTRIM(parameter2.data_files))'+CHR(13)+CHR(10)+ ;
'IF FILE("K:\most-root\program files\most\databases\oms.dbc")'+CHR(13)+CHR(10)+ ;
' path_to_data = "K:\most-root\program files\most\databases\"'+CHR(13)+CHR(10)+ ;
' IF RLOCK("parameter2")'+CHR(13)+CHR(10)+ ;
'  REPLACE parameter2.data_files WITH path_to_data'+CHR(13)+CHR(10)+ ;
'  UNLOCK IN parameter2'+CHR(13)+CHR(10)+ ;
' ENDIF'+CHR(13)+CHR(10)+ ;
'ENDIF',1,1,1)
lcOld='normalized_version("1.7.644")) ;'
lcNew='normalized_version("1.7.644"), normalized_version("1.7.645")) ;'
* Keep the VFP INLIST argument count within its legacy limit by dropping the oldest code-only build.
lcText=STRTRAN(lcText,'normalized_version("1.7.620"), normalized_version("1.7.621")','normalized_version("1.7.621")',1,1,1)
IF lcOld $ lcText
 STRTOFILE(STRTRAN(lcText,lcOld,lcNew,1,1,1),lcOms,0)
ELSE
 IF !('normalized_version("1.7.645")' $ lcText)
...
SCAN
 ...
 IF "V16.43" $ lcProps
  REPLACE properties WITH STRTRAN(lcProps,"V16.43","V16.44",1,-1,1) IN v36version
 ENDIF
 IF "V16.43" $ lcMethods
  REPLACE methods WITH STRTRAN(lcMethods,"V16.43","V16.44",1,-1,1) IN v36version
 ENDIF
ENDSCAN
USE IN v36version
...
PROCEDURE Click
* V16.44 POSITION CLAIMS TO THE RIGHT OF PATIENTS
...
IF FOUND() AND "V16.35 SINGLE LETTERBUILDER" $ v36patients.methods
 REPLACE methods WITH STRTRAN(v36patients.methods,"V16.35 SINGLE LETTERBUILDER","V16.44 SINGLE LETTERBUILDER",1,1,1) IN v36patients
ENDIF
USE IN v36patients
...
BUILD EXE (lcOutput+"MOST_V16_44_XP_K_DATABASE_PATH.exe") FROM "most.pjx" RECOMPILE
STRTOFILE("SUCCESS V16.44 1.7.645 "+TTOC(DATETIME(),1)+CHR(13)+CHR(10),lcLog,0)
ON ERROR
QUIT
...
MESSAGEBOX(lcError,16,"MOSt V16.44 build")
CANCEL
ENDPROC

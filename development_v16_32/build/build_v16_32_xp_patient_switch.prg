...
lcBackup=lcRoot+"backup_v16_32_xp_patient_switch_20260913\"
lcLog=lcOutput+"build_v16_32_xp_patient_switch.log"
...
ENDIF
COPY FILE "C:\Users\abudn\Documents\Codex\2026-09-12\wh\work\openletterwriter_v16_32.prg" TO (lcPrograms+"openletterwriter.prg")

lcOms=lcPrograms+"oms.prg"
lcText=FILETOSTR(lcOms)
lcOld='normalized_version("1.7.625"), normalized_version("1.7.626"), normalized_version("1.7.627"), normalized_version("1.7.628"), normalized_version("1.7.629"), normalized_version("1.7.630"), normalized_version("1.7.631"), normalized_version("1.7.632")) ;'
lcNew=STRTRAN(lcOld,'normalized_version("1.7.632")) ;','normalized_version("1.7.632"), normalized_version("1.7.633")) ;')
IF lcOld $ lcText
    STRTOFILE(STRTRAN(lcText,lcOld,lcNew,1,1,1),lcOms,0)
ELSE
    IF !('normalized_version("1.7.633")' $ lcText)
        ERROR "Version guard could not be patched"
    ENDIF
ENDIF

USE (lcForms+"version.scx") EXCLUSIVE ALIAS v32version
SCAN
    lcProps=v32version.properties
    lcMethods=v32version.methods
    IF "V16.31" $ lcProps
        REPLACE properties WITH STRTRAN(lcProps,"V16.31","V16.32",1,-1,1) IN v32version
    ENDIF
    IF "V16.31" $ lcMethods
        REPLACE methods WITH STRTRAN(lcMethods,"V16.31","V16.32",1,-1,1) IN v32version
    ENDIF
ENDSCAN
USE IN v32version

* Replace only the patient-screen Letters button Click method.
USE (lcForms+"patients.scx") EXCLUSIVE ALIAS v32patients
LOCATE FOR UPPER(ALLTRIM(v32patients.objname))=="LETTERS" AND UPPER(ALLTRIM(v32patients.parent))=="PATIENTS"
IF !FOUND()
    ERROR "Patient-screen Letters button was not found"
ENDIF
lcMethods=v32patients.methods
IF "V16.31 SINGLE LETTERBUILDER" $ lcMethods
    lcMethods=STRTRAN(lcMethods,"V16.31 SINGLE LETTERBUILDER","V16.32 SINGLE LETTERBUILDER",1,1,1)
    lcMethods=STRTRAN(lcMethods,"DO openletterwriter WITH patients.id","DO openletterwriter WITH patients.id, THISFORM",1,1,1)
    REPLACE methods WITH lcMethods IN v32patients
ELSE
IF !("V16.32 SINGLE LETTERBUILDER" $ lcMethods)
...
PROCEDURE Click
* V16.32 SINGLE LETTERBUILDER - open the form only, scoped to current patient.
...
ENDPROC
    ENDTEXT
    REPLACE methods WITH LEFT(lcMethods,lnStart-1)+lcNewClick+SUBSTR(lcMethods,lnEnd+1) IN v32patients
ENDIF
ENDIF
USE IN v32patients

* Route Patients > Letter Builder through the same singleton launcher.
USE (lcMenu+"main_menu.mnx") EXCLUSIVE ALIAS v32menu
LOCATE FOR UPPER(ALLTRIM(v32menu.prompt))=="LETTER BUILDER" AND UPPER(ALLTRIM(v32menu.levelname))=="PATIENTS"
IF !FOUND()
    ERROR "Patients menu Letter Builder item was not found"
ENDIF
REPLACE proctype WITH 1, procedure WITH "DO openletterwriter" IN v32menu
USE IN v32menu

* Confirm the preceding XP lock repair is still present.
USE (lcForms+"letterform.scx") SHARED ALIAS v32lettercheck
LOCATE FOR UPPER(ALLTRIM(v32lettercheck.objname))=="WORD" AND "PAGE1" $ UPPER(v32lettercheck.parent)
IF !FOUND() OR !("V16.29 XP MULTI-WORKSTATION LETTER LOCK" $ v32lettercheck.methods)
    ERROR "V16.29 LetterBuilder lock repair is missing"
ENDIF
USE IN v32lettercheck

CD (lcMost)
_GENMENU=lcRoot+"genmenu.prg"
SET DEFAULT TO (lcMost)
BUILD EXE (lcOutput+"MOST_V16_32_XP_PATIENT_SWITCH.exe") FROM "most.pjx" RECOMPILE
STRTOFILE("SUCCESS V16.32 1.7.633 "+TTOC(DATETIME(),1)+CHR(13)+CHR(10),lcLog,0)
ON ERROR
QUIT
...
MESSAGEBOX(lcError,16,"MOSt V16.32 build")
CANCEL
ENDPROC

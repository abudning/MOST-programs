...
# V16.10: reconcile every fee-table row against an indexed Ministry cursor.
# The legacy SEEK loop updates only one matching row; this pass guarantees that
# every existing server fee row whose first four characters match is refreshed.
reconcile_block = """* V16.10: final authoritative reconciliation with the Ministry master.
SELECT fee_import_cursor
INDEX ON LEFT(DATA,4) TAG feesvc
...
SCAN
    ...
    IF SEEK(lcReconService, \"fee_import_cursor\", \"feesvc\")
...
"""
if "V16.10: final authoritative reconciliation" not in text:
    text = text.replace("SELECT fees\nlnFeeCountAfter = RECCOUNT(\"fees\")", reconcile_block + "SELECT fees\nlnFeeCountAfter = RECCOUNT(\"fees\")", 1)
text = text.replace('"New fee codes added: " + TRANSFORM(lnFeeCountAfter-lnFeeCountBefore) + CHR(13) + ;\n    "Database: " + path_to_data', '"New fee codes added: " + TRANSFORM(lnFeeCountAfter-lnFeeCountBefore) + CHR(13) + ;\n    "Existing fee rows synchronized: " + TRANSFORM(lnFeesReconciled) + CHR(13) + ;\n    "Database: " + path_to_data', 1)
# V16.9: read the version from the executable actually being run. Test builds
# intentionally have descriptive filenames and may not sit beside a MOSt.exe.
old_version_read = """LOCAL ARRAY aCurrentExeVersion(1)
m_current_most_ver = ''
IF AGETFILEVERSION(aCurrentExeVersion, gc_localapp + "MOSt.exe") > 0
\tm_current_most_ver = ALLTRIM(aCurrentExeVersion(4))
ENDIF"""
new_version_read = """LOCAL ARRAY aCurrentExeVersion(1)
LOCAL lcRunningExe
...
\tm_current_most_ver = ALLTRIM(aCurrentExeVersion(4))
ENDIF"""
if old_version_read in text:
    text = text.replace(old_version_read, new_version_read, 1)
...
# Apply V16.10 reconciliation to the fee source after all prior fee patches.
if "V16.10: final authoritative reconciliation" not in text:
    text = text.replace("SELECT fees\nlnFeeCountAfter = RECCOUNT(\"fees\")", reconcile_block + "SELECT fees\nlnFeeCountAfter = RECCOUNT(\"fees\")", 1)
text = text.replace('"New fee codes added: " + TRANSFORM(lnFeeCountAfter-lnFeeCountBefore) + CHR(13) + ;\n    "Database: " + path_to_data', '"New fee codes added: " + TRANSFORM(lnFeeCountAfter-lnFeeCountBefore) + CHR(13) + ;\n    "Existing fee rows synchronized: " + TRANSFORM(lnFeesReconciled) + CHR(13) + ;\n    "Database: " + path_to_data', 1)

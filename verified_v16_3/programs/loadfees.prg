...
USE SUBMITED IN 0
USE CLAIMS IN 0
USE m.MOStTempFolder+"tempfee.dbf" IN 0
...
DO WHILE .NOT. EOF(8)
	...
	ENDIF
	SELECT 9
	servicecode = SUBSTR(temp.DATA,1,4)  && should be alpha followed by 3 numerics
...
* increment number
				...
				m_proffee = VAL(SUBSTR(temp.DATA,54,11))/10000
				IF m_proffee <= 0 THEN               &&  if it's blank use the last fee for profes fee
					m.Proffe2 = VAL(SUBSTR(temp.DATA,65,11))/10000
				ELSE
					m.Proffe2 = m_proffee
				ENDIF

				m.Effective2 = fixfeedate(SUBSTR(temp->DATA,7,6))
				m.termination2 = fixfeedate(SUBSTR(temp->DATA,15,6))
				m.gpFeerate2 = VAL(SUBSTR(temp->DATA,21,11))/10000
				m.TechFee2 = VAL(SUBSTR(temp->DATA,32,11))/10000
				m.SpFeeRate2 = VAL(SUBSTR(temp->DATA,43,11))/10000
...
* end transaction here
			ELSE && answer NO
				...
				REPLACE fees->service WITH SUBSTR(temp->DATA,1,4)
				REPLACE fees->effective WITH fixfeedate(SUBSTR(temp->DATA,7,6))  &&NB if dates are invalid eg 999999 then returns null date
				REPLACE fees->termination WITH fixfeedate(SUBSTR(temp->DATA,15,6))
				REPLACE fees->gpfeerate WITH VAL(SUBSTR(temp->DATA,21,11))/10000
				REPLACE fees->techfee WITH VAL(SUBSTR(temp->DATA,32,11))/10000
				REPLACE fees->spfeerate WITH VAL(SUBSTR(temp->DATA,43,11))/10000
				m_proffee = VAL(SUBSTR(temp->DATA,54,11))/10000
				IF m_proffee <= 0 THEN               &&  if it's blank use the last fee for profes fee
					REPLACE fees->proffee WITH VAL(SUBSTR(temp->DATA,65,11))/10000
				ELSE
...
USE m.MOStTempFolder+"tempfee.dbf"
IF RECCOUNT() > 0
** show report
...
WAIT CLEA
MESSAGEBOX("Fee codes updating completed !",64,"MOSt")

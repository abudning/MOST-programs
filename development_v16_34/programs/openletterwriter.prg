...
* First complete ID-and-Enter pass.
...
KEYBOARD "{ENTER}"
DOEVENTS
* Legacy LetterBuilder sometimes completes only the patient heading on the first
* injected Enter. Repeat the same complete ID-and-Enter pass when no list loaded.
IF toLetterForm.pageframe1.page1.list_letters.ListCount=0
    toLetterForm.pageframe1.page1.id.Click()
    toLetterForm.pageframe1.page1.search.Value=lcId
    toLetterForm.pageframe1.page1.search.SetFocus()
    KEYBOARD "{ENTER}"

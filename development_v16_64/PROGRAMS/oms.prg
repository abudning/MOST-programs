...
IF !FILE(path_to_data + "oms.dbc")
	...
	*** 2003.11.24 try to remap network drive store in most.ne
	...
	IF !bMap_Success && not map
		...
		IF answer = 6
			...
			ELSE
				...
				DO WHILE .T. && In VFP everything is executed synchronously, so the code doesn't wai
...
* tables below are opened by their full configured path; changing the defaul
	...
	IF !(server_vsn == m_current_most_ver) AND !llV1615CodeOnly AND !llLegacySchemaVersion
		IF normalized_version(server_vsn) < normalized_version(m_current_most_ver)
			IF (server_database_ver = local_database_ver) ;
				...
				** but its MOSt.Exe version remained the old one. So both server and clien
			...
			ELSE
				...
				** MOSt is launched from the server, being an older version, it won'
...
&& strDrive - drive to search e.i. "K:"
	...
	IF m.oColDrives.COUNT > 0
		FOR i = 0 TO m.oColDrives.COUNT - 1 STEP 2
			*!* Windows 2000 adds "Network Places" to lis

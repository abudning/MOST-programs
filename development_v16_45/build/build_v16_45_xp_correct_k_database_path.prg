...
lcBackup=lcRoot+"backup_v16_45_xp_correct_k_database_path_20260913\"
lcLog=lcOutput+"build_v16_45_xp_correct_k_database_path.log"
...
COPY FILE "C:\Users\abudn\Documents\Codex\2026-09-12\wh\work\openletterwriter_v16_45.prg" TO (lcPrograms+"openletterwriter.prg")
COPY FILE "C:\Users\abudn\Documents\Codex\2026-09-12\wh\work\letterpatientreloadtimer_v16_45.prg" TO (lcPrograms+"letterpatientreloadtimer.prg")
COPY FILE "C:\Users\abudn\Documents\Codex\2026-09-12\wh\work\rxcenter_v16_45.prg" TO (lcPrograms+"rxcenter.prg")
...
* V16.45: K: is already mapped to the most-Root share; do not repeat the share name.
lcText=STRTRAN(lcText,'K:\most-root\program files\most\databases\','K:\Program Files\MOSt\databases\',1,-1,1)
lcOld='normalized_version("1.7.645")) ;'
lcNew='normalized_version("1.7.645"), normalized_version("1.7.646")) ;'
* Keep the VFP INLIST argument count within its legacy limit by dropping the oldest code-only build.
lcText=STRTRAN(lcText,'normalized_version("1.7.621"), normalized_version("1.7.622")','normalized_version("1.7.622")',1,1,1)
IF lcOld $ lcText
 STRTOFILE(STRTRAN(lcText,lcOld,lcNew,1,1,1),lcOms,0)
ELSE
 IF !('normalized_version("1.7.646")' $ lcText)
...
SCAN
 ...
 IF "V16.44" $ lcProps
  REPLACE properties WITH STRTRAN(lcProps,"V16.44","V16.45",1,-1,1) IN v36version
 ENDIF
 IF "V16.44" $ lcMethods
  REPLACE methods WITH STRTRAN(lcMethods,"V16.44","V16.45",1,-1,1) IN v36version
 ENDIF
ENDSCAN
USE IN v36version
...
PROCEDURE Click
* V16.45 POSITION CLAIMS TO THE RIGHT OF PATIENTS
...
IF FOUND() AND "V16.35 SINGLE LETTERBUILDER" $ v36patients.methods
 REPLACE methods WITH STRTRAN(v36patients.methods,"V16.35 SINGLE LETTERBUILDER","V16.45 SINGLE LETTERBUILDER",1,1,1) IN v36patients
ENDIF
USE IN v36patients
...
BUILD EXE (lcOutput+"MOST_V16_45_XP_CORRECT_K_DATABASE_PATH.exe") FROM "most.pjx" RECOMPILE
STRTOFILE("SUCCESS V16.45 1.7.646 "+TTOC(DATETIME(),1)+CHR(13)+CHR(10),lcLog,0)
ON ERROR
QUIT
...
MESSAGEBOX(lcError,16,"MOSt V16.45 build")
CANCEL
ENDPROC

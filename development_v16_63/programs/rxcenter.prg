* MOSt V16.28 Prescription Centre
* Repairs XP medication printing, Enter-to-view history, and signed optical values.
...
FUNCTION RxPrescriberName
LPARAMETERS tcValue
LOCAL lcValue
lcValue=UPPER(ALLTRIM(TRANSFORM(tcValue)))
IF lcValue=="HH" OR "HAZARI" $ lcValue
    RETURN "Hassan Hazari MD FRCS(C) CPSO 121051"
ENDIF
RETURN "Andrew Budning MD FRCSC CPSO 60349"
ENDFUNC

FUNCTION RxPrescriberPrint
LPARAMETERS tcValue
IF "HAZARI" $ UPPER(ALLTRIM(TRANSFORM(tcValue)))
    RETURN "Hassan Hazari MD, FRCS(C)"
ENDIF
RETURN "Andrew Budning MD, FRCSC"
ENDFUNC

FUNCTION RxPrescriberCPSO
LPARAMETERS tcValue
IF "HAZARI" $ UPPER(ALLTRIM(TRANSFORM(tcValue)))
    RETURN "CPSO 121051"
ENDIF
...
lnValue=RxNumber(tvValue)
lcValue=ALLTRIM(STR(lnValue,8,2))
IF lnValue>=0
    lcValue="+"+lcValue
ENDIF
RETURN lcValue
...
LPARAMETERS tcSource, tdChartDate, tnODSph, tnODCyl, tnODAxis, tnOSSph, tnOSCyl, tnOSAxis, tnODAdd, tnOSAdd
...
ELSE
    ...
    FOR lnP=loDoc.Paragraphs.Count TO 1 STEP -1
        ...
        IF UPPER(LEFT(lcText,10))=="REFRACTION"
            ...
            lnEnd=loDoc.Paragraphs(MIN(lnP+6,loDoc.Paragraphs.Count)).Range.End
...
IF !llOD OR !llOS
    ...
    RETURN .F.
ENDIF
=RxParseAdd(lcBlock,@tnODAdd,@tnOSAdd)
...
* Word may insert arbitrary spaces or tabs inside optical values; ignore them.
lcClean=STRTRAN(STRTRAN(lcClean," ",""),CHR(9),"")
* PL and PLANO are standard optical equivalents of a zero sphere.
lcClean=STRTRAN(lcClean,tcEye+":PLANO",tcEye+":0.00",1,-1,1)
lcClean=STRTRAN(lcClean,tcEye+"PLANO",tcEye+"0.00",1,-1,1)
lcClean=STRTRAN(lcClean,tcEye+":PL",tcEye+":0.00",1,-1,1)
lcClean=STRTRAN(lcClean,tcEye+"PL",tcEye+"0.00",1,-1,1)
...
ELSE
    * Accept sphere-only lines such as OD:+0.50 and OS:+0.50.
    loRE.Pattern="(^|[\r\n])"+tcEye+":?([+-]?\d+(\.\d+)?)([\r\n]|$)"
    loMatches=loRE.Execute(lcClean)
    IF loMatches.Count=0
        RETURN .F.
...
FUNCTION RxParseAdd
LPARAMETERS tcText, tnODAdd, tnOSAdd
LOCAL loRE, loMatches, loMatch, lcClean, lnAdd
lcClean=STRTRAN(STRTRAN(tcText,CHR(150),"-"),CHR(151),"-")
lcClean=STRTRAN(STRTRAN(lcClean," ",""),CHR(9),"")
loRE=CREATEOBJECT("VBScript.RegExp")
loRE.Global=.F.
loRE.IgnoreCase=.T.
loRE.MultiLine=.T.
* Accept ADD +2.50 OU (with or without spaces or a colon).
loRE.Pattern="(^|[\r\n])ADD:?([+]?[0-9]+([.][0-9]+)?)(OU)?([\r\n]|$)"
loMatches=loRE.Execute(lcClean)
IF loMatches.Count=0
    RETURN .F.
ENDIF
loMatch=loMatches.Item(0)
lnAdd=VAL(loMatch.SubMatches(1))
IF !BETWEEN(lnAdd,0,10)
    RETURN .F.
ENDIF
tnODAdd=lnAdd
tnOSAdd=lnAdd
...
DEFINE CLASS RxMedicationForm AS Form
    ...
    ADD OBJECT cboMD AS ComboBox WITH Top=36,Left=278,Width=285,Height=22,Style=2,RowSourceType=1,RowSource="Andrew Budning MD FRCSC CPSO 60349,Hassan Hazari MD FRCS(C) CPSO 121051",Value="Andrew Budning MD FRCSC CPSO 60349"
    ...
    PROCEDURE LoadPatient
        ...
        THIS.cboMD.Value=RxPrescriberName(THIS.oPatient.combo2.Value)
        IF !EMPTY(THIS.cRxId)
            THIS.LoadExisting()
        ENDIF
    ENDPROC
    ...
    PROCEDURE LoadExisting
        ...
        SELECT rxmview
        LOCATE FOR rxmview.RX_ID==THIS.cRxId
        IF FOUND()
            THIS.txtIssue.Value=rxmview.issue_date
            THIS.cboMD.Value=RxPrescriberName(rxmview.primary_md)
    ...
    PROCEDURE SaveRx
        ...
        REPLACE rx_id WITH lcId, pat_id WITH INT(THIS.oPatient.id1.Value), issue_date WITH THIS.txtIssue.Value, ;
            primary_md WITH RxPrescriberCode(THIS.cboMD.Value), status WITH UPPER(tcStatus), drug_name WITH ALLTRIM(THIS.txtDrug.Value), ;
    ...
    PROCEDURE PreviewRx
        ...
        lcFile=ADDBS(SYS(2023))+"MOST_Medication_Rx_"+SYS(2015)+".htm"
        lcStatus=IIF(THIS.lReadOnly,"ISSUED","DRAFT - NOT A VALID PRESCRIPTION")
        lcHtml="<html><head><title>Medication Prescription</title>"
        lcHtml=lcHtml+"<style>body{font-family:Arial;margin:20px}.status{font-weight:bold;color:#900}"
        lcHtml=lcHtml+".office{text-align:center;line-height:1.35;margin-bottom:8px}.sign{margin-top:25px;border-top:1px solid #000;width:320px}</style></head><body>"
        ...
        lcHtml=lcHtml+"<h1>Medication Prescription</h1><p class='status'>"+RxHtml(lcStatus)+"</p>"
        lcHtml=lcHtml+"<p><b>Patient:</b> "+RxHtml(THIS.cPatientName)+" &nbsp; <b>Date of birth:</b> "+RxHtml(THIS.cPatientDOB)+"</p>"
        lcHtml=lcHtml+"<p><b>Date:</b> "+RxHtml(DTOC(THIS.txtIssue.Value))+"</p>"
        ...
        lcHtml=lcHtml+"<h2>"+RxHtml(THIS.txtDrug.Value)+" "+RxHtml(THIS.txtStrength.Value)+"</h2>"
        lcHtml=lcHtml+"<p><b>Form:</b> "+RxHtml(THIS.txtForm.Value)+" &nbsp; <b>Route:</b> "+RxHtml(THIS.txtRoute.Value)+"</p>"
        lcHtml=lcHtml+"<p><b>Directions:</b> "+RxHtml(THIS.edtDirections.Value)+"</p>"
        lcHtml=lcHtml+"<p><b>Quantity:</b> "+RxHtml(THIS.txtQty.Value)+" &nbsp; <b>Repeats:</b> "+RxHtml(THIS.txtRepeats.Value)+IIF(THIS.chkNoSub.Value," &nbsp; <b>No substitution</b>","")+"</p>"
        lcHtml=lcHtml+"<p><b>Notes:</b> "+RxHtml(THIS.edtNotes.Value)+"</p>"
        lcHtml=lcHtml+"<p class='sign'>Prescriber signature</p><p>"+RxHtml(RxPrescriberPrint(THIS.cboMD.Value))+"<br><b>"+RxHtml(RxPrescriberCPSO(THIS.cboMD.Value))+"</b></p><small>MOSt Rx V16.28</small></body></html>"
...
DEFINE CLASS RxSignedTextBox AS TextBox
    Value="+0.00"
    Alignment=1
    PROCEDURE Valid
        THIS.Value=RxSigned(THIS.Value)
        RETURN .T.
    ENDPROC
ENDDEFINE
...
DEFINE CLASS RxGlassesForm AS Form
    ...
    PROCEDURE LoadPatient
        ...
        THIS.cboMD.Value=RxPrescriberName(THIS.oPatient.combo2.DisplayValue)
        IF !EMPTY(THIS.cRxId)
            THIS.LoadExisting()
        ENDIF
    ENDPROC
    ...
    PROCEDURE LoadExisting
        ...
        SELECT rxview
        LOCATE FOR rxview.RX_ID==THIS.cRxId
        IF FOUND()
            THIS.txtIssue.Value=rxview.issue_date
            THIS.txtExam.Value=rxview.exam_date
            THIS.cboMD.Value=RxPrescriberName(rxview.primary_md)
            THIS.txtODSph.Value=RxSigned(rxview.od_sph)
            THIS.txtODCyl.Value=RxSigned(rxview.od_cyl)
            THIS.txtODAxis.Value=rxview.od_axis
            THIS.txtOSSph.Value=RxSigned(rxview.os_sph)
            THIS.txtOSCyl.Value=RxSigned(rxview.os_cyl)
            THIS.txtOSAxis.Value=rxview.os_axis
            THIS.txtODAdd.Value=RxSigned(rxview.od_add)
            THIS.txtOSAdd.Value=RxSigned(rxview.os_add)
    ...
    PROCEDURE SaveRx
        ...
        ENDIF
        IF UPPER(tcStatus)=="ISSUED"
            IF EMPTY(THIS.cboMD.Value) OR EMPTY(THIS.txtIssue.Value)
        ...
        REPLACE rx_id WITH lcId, pat_id WITH INT(THIS.oPatient.id1.Value), ;
            ...
            primary_md WITH RxPrescriberCode(THIS.cboMD.Value), status WITH UPPER(tcStatus), ;
    ...
    PROCEDURE PreviewRx
        LOCAL lcFile, lcStatus, lcHtml, lcOD, lcOS, lcAdd, lcPrism, loShell
        lcFile=ADDBS(SYS(2023))+"MOST_Glasses_Rx_"+SYS(2015)+".htm"
        lcStatus=IIF(THIS.lReadOnly,"ISSUED","DRAFT - NOT A VALID PRESCRIPTION")
        ...
        lcPrism=""
        IF RxNumber(THIS.txtODHP.Value)#0 OR RxNumber(THIS.txtODVP.Value)#0 OR ;
            RxNumber(THIS.txtOSHP.Value)#0 OR RxNumber(THIS.txtOSVP.Value)#0
        ...
        lcHtml=lcHtml+"<style>body{font-family:Arial;margin:20px;color:#000}h1{font-size:24px}.status{font-weight:bold;color:#900}"
        lcHtml=lcHtml+"table{border-collapse:collapse;width:100%;margin-top:20px}td.sign{font-weight:bold}th,td{border:1px solid #777;padding:10px;text-align:left}"
        lcHtml=lcHtml+".office{text-align:center;line-height:1.35;margin-bottom:8px}.sign{margin-top:25px;border-top:1px solid #000;width:320px}</style></head><body>"
        lcHtml=lcHtml+"<div class='office'><b>Budning Eye Institute</b><br>2300 Eglinton Avenue West<br>Suite 305<br>Mississauga, Ontario L5M 2V8<br>Tel: 905-820-5464<br>Fax 905-569-2377</div><hr>"
        ...
        ENDIF
        lcHtml=lcHtml+"<p><b>Use:</b> "+RxHtml(THIS.txtUse.Value)+"</p>"
        lcHtml=lcHtml+"<p><b>Notes:</b> "+RxHtml(THIS.edtNotes.Value)+"</p>"
        lcHtml=lcHtml+"<p class='sign'>"+RxHtml(RxPrescriberPrint(THIS.cboMD.Value))+"<br><b>"+RxHtml(RxPrescriberCPSO(THIS.cboMD.Value))+"</b><br>Digitally signed</p><small>MOSt Rx V16.28</small></body></html>"
        STRTOFILE(lcHtml,lcFile,0)
        TRY
            loShell=CREATEOBJECT("WScript.Shell")
            loShell.Run(CHR(34)+lcFile+CHR(34),1,.F.)
...
DEFINE CLASS RxImportButton AS CommandButton
    PROCEDURE Click
        LOCAL lcSource, ldDate, lnODS, lnODC, lnODA, lnOSS, lnOSC, lnOSA, lnODAdd, lnOSAdd
        LOCAL lnI, loWindow, lnActivePatient, lnLetterPatient, lnRxPatient
        lnActivePatient=0
        lnLetterPatient=0
        lnRxPatient=INT(THISFORM.oPatient.id1.Value)
        FOR lnI=1 TO _SCREEN.FormCount
            loWindow=_SCREEN.Forms(lnI)
            IF UPPER(ALLTRIM(loWindow.Caption))=="PATIENTS"
                TRY
                    lnActivePatient=INT(loWindow.keep_id)
                CATCH
                ENDTRY
            ENDIF
            IF UPPER(ALLTRIM(loWindow.Caption))=="LETTER BUILDER"
                TRY
                    lnLetterPatient=VAL(TRANSFORM(loWindow.pageframe1.page1.patient_id.Value))
                CATCH
                ENDTRY
            ENDIF
        ENDFOR
        IF (lnActivePatient>0 AND lnActivePatient#lnRxPatient) OR ;
            (lnLetterPatient>0 AND lnLetterPatient#lnRxPatient)
            MESSAGEBOX("The active patient chart does not match the glasses Rx patient. Select the correct patient before importing.",48,"MOSt - Patient Mismatch")
            RETURN
        ENDIF
        IF RxReadWordRefraction(@lcSource,@ldDate,@lnODS,@lnODC,@lnODA,@lnOSS,@lnOSC,@lnOSA,@lnODAdd,@lnOSAdd)
            IF MESSAGEBOX("Complete refraction found:"+CHR(13)+;
                "OD "+TRANSFORM(lnODS)+" "+TRANSFORM(lnODC)+" x "+TRANSFORM(lnODA)+CHR(13)+;
                "OS "+TRANSFORM(lnOSS)+" "+TRANSFORM(lnOSC)+" x "+TRANSFORM(lnOSA)+CHR(13)+;
                IIF(lnODAdd>0,"ADD "+TRANSFORM(lnODAdd)+" OU"+CHR(13),"")+CHR(13)+;
                "Copy these values into the glasses prescription?",4+32+256,"MOSt - Confirm Refraction")=6
                THISFORM.txtODSph.Value=RxSigned(lnODS)
                THISFORM.txtODCyl.Value=RxSigned(lnODC)
                THISFORM.txtODAxis.Value=lnODA
                THISFORM.txtOSSph.Value=RxSigned(lnOSS)
                THISFORM.txtOSCyl.Value=RxSigned(lnOSC)
                THISFORM.txtOSAxis.Value=lnOSA
                IF lnODAdd>0
                    THISFORM.txtODAdd.Value=RxSigned(lnODAdd)
                    THISFORM.txtOSAdd.Value=RxSigned(lnOSAdd)
                ENDIF
...
DEFINE CLASS RxHistoryGrid AS Grid
    PROCEDURE KeyPress
        LPARAMETERS nKeyCode, nShiftAltCtrl
        IF nKeyCode=13
            NODEFAULT
            THISFORM.cmdView.Click()
        ENDIF
    ENDPROC
ENDDEFINE
...
DEFINE CLASS RxHistoryForm AS Form
    ...
    ADD OBJECT grdHistory AS RxHistoryGrid WITH Top=35,Left=10,Width=680,Height=300,RecordSource="",DeleteMark=.F.,AllowAddNew=.F.,AllowDelete=.F.,AllowCellSelection=.F.,ReadOnly=.T.
    ...
    PROCEDURE LoadHistory
        LOCAL lcRxFile, lcMedFile, lcPresFile, lcGenFile, lcBrandFile, lnPatId
        THIS.lblPatient.Caption="Patient: "+RxPatientName(THIS.oPatient)+"     Date of birth: "+RxPatientDOB(THIS.oPatient)
        lnPatId=INT(THIS.oPatient.id1.Value)
        ...
        IF FILE(lcRxFile)
            ...
            SELECT rxh
            SCAN FOR rxh.pat_id=lnPatId
                INSERT INTO csrRxHistory VALUES (DTOT(rxh.issue_date),"GLASSES",;
                    "OD "+RxSigned(rxh.od_sph)+" "+RxSigned(rxh.od_cyl)+"x"+TRANSFORM(rxh.od_axis)+;
                    " / OS "+RxSigned(rxh.os_sph)+" "+RxSigned(rxh.os_cyl)+"x"+TRANSFORM(rxh.os_axis),;

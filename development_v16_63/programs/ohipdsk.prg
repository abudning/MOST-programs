...
LOCAL m_recordno, m_accounting, m_facility, m_add_title, m_header_location
m_recordno=0
m_accounting=0
m_facility=0
...
&&   @ 2,5 say 'Provider: ' + md->surname + '  ' + md->ohipnumber+ '   '+ md->group_num
	...
	IF TRIM(location)=='' THEN
		location= SPACE(4)
	ENDIF
	* Office visual-field professional claims use the Ministry's OFF SLI.
	* The provider does not need to maintain a separate location code for G432A;
	* the encounter header is adjusted below when that service is encountered.
...
&& of resubmission
					...
					IF claims.accounting<>last_accounting
						m_header_location=location
						IF EMPTY(ALLTRIM(m_header_location))
							* SLI is an encounter-header value. Check every service in
							* this accounting group so G432A is covered even when it is
							* not the first line in the group.
							m_recordno=RECNO()
							SCAN FOR claims.accounting==m_accounting
								IF UPPER(ALLTRIM(claims.service))=='G432A'
									m_header_location='OFF '
									EXIT
								ENDIF
							ENDSCAN
							GOTO m_recordno
						...
						*** END sequence of code added on 2001.08.13 to correctly deal with the facility for a batch of claims. GEORGE
						...
						REPLACE DATA WITH 'HEH'+m_hc_ver+birthday+ accountg + m_claim_type + 'P' ;
							...
							+ claims->manreview + m_header_location + SPACE(17)

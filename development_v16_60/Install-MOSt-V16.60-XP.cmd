...
echo MOSt V16.60 - version 1.7.661 - XP TEST CHECKPOINT
...
:choosebackup
set "BACKUP=%TARGET%\Upgrade_Backup_V16_60_%RANDOM%_%RANDOM%"
...
if errorlevel 1 goto restore
echo.
echo SUCCESS: V16.60 / 1.7.661 installed.
echo Previous executable saved in "%BACKUP%".
echo Database files and workstation configuration were not changed.
echo Follow TEST_NOTES.md before approving this test build for live use.
pause
exit /b 0
:restore
